...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const _ctaL3Def={comment:"and I'll send it straight to you",follow:"for more content like this — posted daily",save:"so you don't lose it",share:"with someone who needs to see this",like:"if this helped you"};
    const _ctaTopDef=isDarkFade?"Save this — your future self will thank you.":isListicle?"Want more content like this?":isCleanPro?"Want the full strategy?":isStory?"Want to start your own journey?":isSplit?"Save this for later.":"Want more like this?";
        ...
        {nav==="templates"&&(
          <div style={{animation:"fadeUp 0.3s ease",maxWidth:1100,margin:"0 auto",width:"100%",paddingBottom:60}}>
                  ...
                  <div key={t.id} style={{position:"relative"}}>
                    <div onClick={()=>{
                      ...
                      if(!restored){
                        ...
                        if(t.id==="listicle"){setTmplEffect("fire");setTmplFont("bebasneue");setTmplBg("dark");setTmplPrimary("#FF6600");setTmplSecondary("#ffffff");setTmplAccentLineColor("#FF6600");setTmplFontSize(72);setTmplCtaType("follow");setTmplCtaTopLine("");setTmplCtaLine2("");setTmplCtaKeyword("");setTmplCtaRewardLine("");}
                        else if(t.id==="dark-fade"){setTmplEffect("gold");setTmplFont("bebasneue");setTmplBg("dark");setTmplPrimary("#C9A84C");setTmplSecondary("#ffffff");setTmplAccentLineColor("#C9A84C");setTmplFontSize(72);setTmplCtaType("save");setTmplCtaTopLine("");setTmplCtaLine2("");setTmplCtaKeyword("");setTmplCtaRewardLine("");}
                        else if(t.id==="clean-pro"){setTmplEffect("none");setTmplFont("inter");setTmplBg("white");setTmplPrimary("#111111");setTmplSecondary("#444444");setTmplAccentLineColor("#BB9900");setTmplFontSize(72);setTmplFontStyle("Inter");setTmplCtaType("comment");setTmplCtaTopLine("");setTmplCtaLine2("");setTmplCtaKeyword("");setTmplCtaRewardLine("");}
            ...
            {tmplSelected&&(()=>{
              ...
              const ctaLine3Defaults={comment:"and I'll send it straight to you",follow:"for more content like this — posted daily",save:"so you don't lose it",share:"with someone who needs to see this",like:"if this helped you"};
              const ctaHTML=tmplShowCta?buildCtaHTML(opts,tmplCtaType,ctaKeywordDefaults[tmplCtaType],tmplCtaTopLine||(isDarkFade?"Save this — your future self will thank you.":_iife_isListicle?"Want more content like this?":isCleanPro?"Want the full strategy?":isStory?"Want to start your own journey?":isSplit?"Save this for later.":"Want more like this?"),tmplCtaLine2||ctaLine2Defaults[tmplCtaType],tmplCtaRewardLine||ctaLine3Defaults[tmplCtaType],ctaBgFinal,name,handle,profileUrl,blueTick,isStory?tmplFontStyle:tmplFont,tmplSlideCount+(tmplShowCta?1:0),tmplShowCounter):null;
                    ...
                    {tmplContentStyleTab==="content"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
                      ...
                      <div style={{background:A.surface,border:`1.5px solid ${tmplShowCta?GOLD:A.border}`,borderRadius:10,overflow:"hidden"}}>
                        ...
                        {tmplShowCta&&<div style={{padding:"0 14px 14px",display:"flex",flexDirection:"column",gap:10}}>
                          ...
                          <input value={tmplCtaTopLine} onChange={e=>setTmplCtaTopLine(e.target.value)} placeholder={isDarkFade?"Save this — your future self will thank you.":isListicle?"Want more content like this?":isCleanPro?"Want the full strategy?":isStory?"Want to start your own journey?":isSplit?"Save this for later.":"Want more like this?"} style={{...inp}}/>
                          <label style={lbl}>Line 2 — Action</label>
                          <input value={tmplCtaLine2} onChange={e=>setTmplCtaLine2(e.target.value)} placeholder={tmplCtaType==="comment"?"Comment the word":tmplCtaType==="follow"?"Follow for more content like this":tmplCtaType==="save"?"Save this for later":tmplCtaType==="share"?"Share this with someone":"Like this if it helped"} style={{...inp}}/>
                          ...
                          <input value={tmplCtaRewardLine} onChange={e=>setTmplCtaRewardLine(e.target.value)} placeholder={tmplCtaType==="comment"?"and I'll send it straight to you":tmplCtaType==="follow"?"for more content like this — posted daily":tmplCtaType==="save"?"so you don't lose it":tmplCtaType==="share"?"with someone who needs to see this":"if this helped you"} style={{...inp}}/>
