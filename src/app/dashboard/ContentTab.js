    ...
    <div style={{ ...card, marginBottom: 20 }}>
      ...
      {media.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <label style={lbl}>Cover photo <span style={{ textTransform: "none", letterSpacing: 0, fontWeight: 500 }}>(optional — least used first)</span></label>
...
function Editor({ api, itemId, onBack, onChanged }) {
  ...
  const load = async () => {
    try { const { item } = await api.get(`/api/content?id=${itemId}`); setItem(item); setDraft(pick(item)); } catch (e) { setErr(e.message); }
  };
  useEffect(() => { load(); }, [itemId]);

  function pick(i) {
    return { idea: i.idea, scheduled_for: i.scheduled_for, slides: (i.slides || []).map((s) => ({ ...s })), caption: i.caption || "", yt_title: i.yt_title || "", yt_tags: (i.yt_tags || []).join(", "), yt_pinned_comment: i.yt_pinned_comment || "", yt_category: i.yt_category || "" };
  }
  ...
  const save = () => run("save", () => api.patch("/api/content", {
    id: item.id, idea: draft.idea, scheduled_for: draft.scheduled_for, slides: draft.slides, caption: draft.caption, yt_title: draft.yt_title,
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 12, color: C.muted }}>{item.brands?.name}</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ ...card, marginBottom: 14 }}>
        ...
        {stale ? <div style={{ fontSize: 11, color: C.gold, marginBottom: 6 }}>Slide text changed — save, then re-render to update the images.</div> : null}
      ...
      <div style={{ ...card, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <label style={{ ...lbl, margin: 0 }}>Caption (Instagram · TikTok · YouTube description)</label>
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => regen("copy")} disabled={!!busy} style={btn("small")}>{busy === "regen-copy" ? <><Spinner /> Rewriting…</> : "↻ Rewrite captions"}</button>
            <CopyButton text={draft.caption} label="Copy" kind="small" />
          </div>
        </div>
        <textarea value={draft.caption} onChange={(e) => setDraft((d) => ({ ...d, caption: e.target.value }))} rows={8} style={{ ...inp, resize: "vertical", lineHeight: 1.6, marginBottom: 12 }} />
      ...
      <div style={card}>
        ...
        <PackageView item={{ ...item, ...draft, yt_tags: draft.yt_tags.split(",").map((t) => t.trim()).filter(Boolean) }} />
      </div>
    </div>
  );
}
