...
export async function POST(req) {
  ...
  const out = { audio: null, statusUrl: null, responseUrl: null, errors: [] };

  try {
    const submitRes = await fetch("https://queue.fal.run/fal-ai/ltx-2/text-to-video/fast", {
      method: "POST",
      headers: FAL_HEADERS,
      body: JSON.stringify({
    ...
    if (!submitRes.ok) throw new Error(`submit ${submitRes.status}: ${submitText.slice(0, 500)}`);
    if (!submitOut.status_url || !submitOut.response_url) throw new Error(`submit returned no status/response URL: ${submitText.slice(0, 300)}`);
    out.statusUrl = submitOut.status_url;
    out.responseUrl = submitOut.response_url;
  } catch (e) {
