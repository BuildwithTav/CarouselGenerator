...
- **Security gap, unresolved as of this doc**: Row Level Security is disabled on all 7 tables. Anyone with the anon key can currently read/write every row, including commissions and payout requests. Do not enable RLS blind — needs policies designed first or it will lock the app out of its own data.
